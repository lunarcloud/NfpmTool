...
2. **Test Job**: Matrix tests the built package across:
   * Operating Systems: ubuntu-latest, windows-latest, ubuntu-24.04-arm, windows-11-arm
   * .NET Version: 10.x

The test job verifies that the packaged nFPM tool can be installed and executed correctly by:

* Installing the tool as a local .NET tool
* Running `nfpm --help` to verify basic functionality
* Running `nfpm --version` to verify version information
* Building real `.deb` and `.rpm` packages from `test/fixtures/nfpm.yaml` to verify nfpm actually works
  end-to-end, and asserting the output files are non-empty
...
2. Downloads nFPM binaries for Windows (x64, arm64) and Linux (x64, arm64)
