...
* Expected behavior
* Actual behavior
* Your environment (OS, .NET version, NfpmTool version)
...
The package tests verify the tool can be installed and executed by:

* Running `nfpm --help` and `nfpm --version`
* Building real `.deb` and `.rpm` packages from the fixture at `test/fixtures/nfpm.yaml` and confirming the
  output files are non-empty

The matrix also includes `ubuntu-24.04-arm` and `windows-11-arm` runners so the bundled arm64 binaries are
actually executed, not just the x64 ones.
